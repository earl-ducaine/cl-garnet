	 ...
	 (let* ((cn
	   ...
	   (when (and (sb-constraint-p cn)
	     ...
	     (setf (getf (sb-constraint-other-slots cn) :mg-os nil)
	       ...
	       (format t "path: ~s~%" (getf (sb-constraint-other-slots cn)
					     :mg-variable-paths nil))
	       (let ((slot1  (nth 0 (getf (sb-constraint-other-slots cn)
					     :mg-variable-paths nil)))
		     (slot2  (nth 1 (getf (sb-constraint-other-slots cn)
					     :mg-variable-paths nil))))
		 (format t "slot1: ~s~%slot2 ~s~%" slot1 slot2)
		 (setf (gethash slot1 *axis-rectangle-hash-table*)
		       nil)
		 (setf (gethash slot2 *axis-rectangle-hash-table*)
		       nil))
