...
(let ((type '((or (is-a-p filling-style) null) :filling-style)))
  (let* ((slot (cadr type))
	 (schema-bins (schema-bins *graphical-object*))
	 (hash (gethash (cadr type) schema-bins)))
    (setf hash (make-sl))
    (setf (sl-name hash) (cadr type))
    (setf (sl-value hash) *no-value*)
    (setf (sl-bits hash) 33)
    (setf (gethash (cadr type) schema-bins) hash)))

(let ((type '((or (is-a-p line-style) null) :line-style)))
  (let* ((slot (cadr type))
	 (schema-bins (schema-bins *graphical-object*))
	 (hash (gethash (cadr type) schema-bins)))
    (setf hash (make-sl))
