;;; -*- mode: lisp; syntax: common-lisp; package: demos-controller -*-
...
(defun Do-Go ()
  ...
  (kr::create-instance 'win1 inter:interactor-window
    (:left 0)
    (:top 240)
    (:width (o-formula (floor (* 1.1 (gv agg1 :height)))))
    (:height (o-formula (floor (* 1.1 (gv agg1 :height)))))
    (:width 10)
    (:height 10)
  ...
  (create-instance 'demos-agg opal:aggregate)
  (create-instance 'apps-agg opal:aggregate)
  ...
  (dolist (button (g-value app-buttons :X-BUTTON-LIST :components))
  ...
  (opal:add-components demos-agg bt qbt demos-mouseline)
  (opal:add-components apps-agg app-buttons)
  (opal:add-components agg1 demos-agg)
  (opal:add-components agg1 apps-agg)
