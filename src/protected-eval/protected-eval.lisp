...
(defvar *user-type* :programmer
  "Set this to :programmer if the user should be allowed to enter the
   debugger in protected evaluation mode.")
...
(defun prompting-error-handler (context condition
  ...
  "Handles an error by prompting the user for abort, debug,
   store-value, use-value or continue.  <allow-debugger> controls
   whether the user is given the option of entering the debugger.

   Note that this function takes two arguments, while hanlers are
   usually meant to take one.  It should be invoked with
   (handler-bind ((error #'(lambda (condition)
       (prompting-error-handler context-string condition))))
       ...)  or the like.

   This function will eventually invoke a restart contindition and
   hence should never return."
