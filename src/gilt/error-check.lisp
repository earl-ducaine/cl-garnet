...
(defun Add-Panel (gadget val)
  ...
  (let* ((main-gadget (g-value gadget :parent))
    ...
    (s-value (g-value error-list :window)
	     :height
	     (+ 10 (or (opal:bottom (g-value main-gadget :another-panel))
		       0)))
...
(defun Remove-Panel-If-Unused (box val)
  ...
  (when (equal "" val)
	...
	;; (opal:update win)
	(s-value win :height (+ 10 (or (opal:bottom (g-value error-list :parent
							     :another-panel))
				       0))))
      (opal:update win))))
...
;; Set up the labeled-box panels with the error values that have
;; already been defined for the object.  The error values are in an
;; association list, i.e., ((val . string) ...).  Iterate over the
;; list of pairs in the association list and set the strings of the
;; labeled boxes in the error check panel.
(defun init-error-check (ec ec-vals)
  (let* ((panel-agglist (g-value ec :panel-list))
    ...
    (s-value (g-value ec :window)
	     :height
	     (+ 5 (or (opal:bottom (g-value ec :another-panel))
		      0)))))
