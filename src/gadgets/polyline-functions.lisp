...
;;; This is the handle with which the user can drag, create, and
;;; delete polyline points.  Basically, it's a 7 by 7 square centered
;;; around the point.  The point is specified by setting the :xy slot
;;; to be the xy pair.  The handle is visible when it is a member of
;;; the :polyline-handles-being-used slot of the polyline creator.
...
;;; This is called by the point-moving interactor when it starts up.
;;; Basically, we have to set the feedback object to be visible, and
;;; then set up the correct attach points for the feedback object.
;;; Then, the interactor will set the :box slot of the feedback-obj
;;; and everything will sail smoothly.
...
;;; If the polyline is closed (first & last points are equal), set the
;;; :closed-polyline-p slot to T
...
(defun hide-polyline-handles (poly-creator)
  (let ((poly (g-value poly-creator :polyline-being-edited)))
    ...
    (opal:update (g-value poly :window))))
;;; Decides what object to show/hide polyline handles over, and calls
;;; make-polyline-handles, which makes the handles
